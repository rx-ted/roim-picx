...
export class HFStorageProvider implements StorageProvider {
  ...
  private async commitDirectFile(
    filePath: string,
    file: Blob,
    commitMessage: string,
  ): Promise<any> {
  ...
  private async verifyFileExists(path: string): Promise<void> {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      if (pathsResp.ok) {
        try {
          ...
          const found =
            Array.isArray(pathsJson) &&
            pathsJson.some(
              (item) => item.path === path || item.path === decodeURIComponent(encodedPath),
            );
          if (found) {
            return;
          }
          ...
          if (
            pathsText.includes(`"path":"${path}"`) ||
            pathsText.includes(`"path":"${encodedPath}"`)
          ) {
            return;
          }
        }
    ...
    throw new Error(
      `HF Verify failed: file not found after commit (${path}). resolveStatus=${lastResolveStatus}, rawStatus=${lastRawStatus}, pathsStatus=${lastPathsStatus}`,
    );
  }
  ...
  async put(key: string, body: any, options: any): Promise<StorageResult> {
    ...
    if (needsLfs) {
      ...
      const commitResult = await this.commitLfsFile(
        safeKey,
        sha256,
        file.size,
        `Upload ${safeKey}`,
  ...
  async delete(key: string): Promise<void> {
    // const url = `${this.baseUrl}/${this.repo}/discussions`; // TODO: What is here?
