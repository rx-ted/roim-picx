...
import { FailCode, NotAuth, Fail, type ApiKey } from '../type';
...
export const auth = async (c: Context<AppEnv>, next: Next) => {
  ...
  // 跳过登录相关路由和配置接口
  if (
    c.req.path.startsWith('/rest/github/login') ||
  ...
  if (token.startsWith('px_')) {
    try {
      ...
      const isAllowed =
        allowedRoutes.some((route) => path === route.path && method === route.method) ||
        (path.startsWith('/rest/del/') && method === 'GET');

      if (!isAllowed) {
        return c.json(
          FailCode(
            'API Key has limited permissions. Only upload, list, and delete are allowed.',
            403,
          ),
        );
      }
      ...
      return c.json(
        FailCode(`API Key validation failed: ${(e as Error).message}`, StatusCode.NotAuth),
      );
    }
  }
