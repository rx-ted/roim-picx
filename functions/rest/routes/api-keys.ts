...
import { Ok, Fail, type ApiKey, type User } from '../type';
...
apiKeyRoutes.post('/api-keys', auth, async (c) => {
  ...
  try {
    ...
    await c.env.DB.prepare(
      ...
      .bind(id, dbUser.id, user.login, prefix, hashHex, name, expires_at || null)
      .run();
...
apiKeyRoutes.delete('/api-keys/:id', auth, async (c) => {
  ...
  try {
    ...
    const key = await c.env.DB.prepare('SELECT user_login FROM api_keys WHERE id = ?')
      .bind(id)
