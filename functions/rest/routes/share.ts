...
shareRoutes.post('/share', auth, async (c) => {
  try {
    ...
    const img = await c.env.DB.prepare(
      'SELECT key, storage_type, expires_at FROM images WHERE key = ?',
    )
      .bind(data.imageKey)
      ...
      c.executionCtx.waitUntil(
        c.env.DB.prepare(
          ...
          .bind(
            ...
            user.login,
            data.imageKey,
            JSON.stringify({ shareId, hasPassword: !!data.password, maxViews: data.maxViews }),
          )
          .run()
...
shareRoutes.get('/share/my', auth, async (c) => {
  ...
  try {
    ...
    const myShares = await Promise.all(
      (results.results || []).map(async (record) => {
...
shareRoutes.get('/share/:id', async (c) => {
  ...
  const record = await c.env.DB.prepare('SELECT * FROM shares WHERE id = ?')
    .bind(shareId)
    .first<DbShare>();
...
shareRoutes.post('/share/:id/verify', async (c) => {
  ...
  await c.env.DB.prepare('UPDATE shares SET current_views = current_views + 1 WHERE id = ?')
    .bind(shareId)
    .run();
...
shareRoutes.delete('/share/:id', auth, async (c) => {
  ...
  const result = await c.env.DB.prepare('DELETE FROM shares WHERE id = ?').bind(shareId).run();
  ...
  if (user) {
    c.executionCtx.waitUntil(
      c.env.DB.prepare(
        ...
        .bind(user.id, user.login, shareId, JSON.stringify({ shareId }))
        .run()
