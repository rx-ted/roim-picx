...
albumRoutes.put('/albums/:id', auth, async (c) => {
  ...
  try {
    ...
    await c.env.DB.prepare(
      ...
      .bind(name, description || null, coverImage || null, enableRandomImage ? 1 : 0, now, id)
      .run();
  ...
  try {
    ...
    const [images, countResult] = await Promise.all([
      c.env.DB.prepare(
        ...
        .all(),
      c.env.DB.prepare('SELECT COUNT(*) as count FROM album_images WHERE album_id = ?')
        .bind(id)
        .first<{ count: number }>(),
    ]);
...
albumRoutes.post('/albums/:id/cover', auth, async (c) => {
  ...
  try {
    const result = await c.env.DB.prepare(
      ...
      .bind(coverImage, Date.now(), id, user!.id)
      .run();
...
albumRoutes.post('/albums/:id/share', auth, async (c) => {
  ...
  try {
    ...
    const existingShare = await c.env.DB.prepare('SELECT id FROM album_shares WHERE album_id = ?')
      .bind(id)
      .first<{ id: string }>();
    ...
    c.executionCtx.waitUntil(
      c.env.DB.prepare(
        ...
        .bind(
          user!.id,
          user!.login,
          JSON.stringify({ albumId: id, shareId, action: existingShare ? 'update' : 'create' }),
        )
        .run(),
    );
...
albumRoutes.post('/share/album/:token/verify', async (c) => {
  ...
  try {
    const share = await c.env.DB.prepare('SELECT * FROM album_shares WHERE id = ?')
      .bind(token)
      .first<any>();
    ...
    // Increment views
    await c.env.DB.prepare('UPDATE album_shares SET current_views = current_views + 1 WHERE id = ?')
      .bind(token)
      .run();
