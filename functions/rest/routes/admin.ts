...
import { auth, type AppEnv, adminAuth } from '../middleware/auth';
...
adminRoutes.get('/users/:login', auth, adminAuth, async (c) => {
  ...
  try {
    const user = await c.env.DB.prepare('SELECT * FROM users WHERE login = ?')
      .bind(login)
      .first<DbUser>();
...
adminRoutes.post('/users/:login/view-all', auth, adminAuth, async (c) => {
  ...
  try {
    const result = await c.env.DB.prepare('UPDATE users SET can_view_all = ? WHERE login = ?')
      .bind(grant ? 1 : 0, login)
      .run();
...
adminRoutes.post('/users/:login/role', auth, adminAuth, async (c) => {
  ...
  try {
    const result = await c.env.DB.prepare(
      ...
      .bind(role, role === 'admin' ? 1 : 0, login)
      .run();
...
adminRoutes.post('/users/:login/quota', auth, adminAuth, async (c) => {
  ...
  try {
    const result = await c.env.DB.prepare('UPDATE users SET storage_quota = ? WHERE login = ?')
      .bind(quota, login)
      .run();
...
adminRoutes.get('/stats', auth, adminAuth, async (c) => {
  try {
    const [userCount, imageCount, totalSize, recentUploads] = await Promise.all([
      ...
      c.env.DB.prepare('SELECT COALESCE(SUM(size), 0) as total FROM images').first<{
        total: number;
      }>(),
      c.env.DB.prepare(
...
adminRoutes.get('/users/:login/stats', auth, adminAuth, async (c) => {
  ...
  try {
    const [imageCount, totalSize, totalViews, recentUploads] = await Promise.all([
      c.env.DB.prepare('SELECT COUNT(*) as count FROM images WHERE user_login = ?')
        .bind(login)
        .first<{ count: number }>(),
      c.env.DB.prepare('SELECT COALESCE(SUM(size), 0) as total FROM images WHERE user_login = ?')
        .bind(login)
        .first<{ total: number }>(),
      c.env.DB.prepare(
...
adminRoutes.post('/sync-r2-to-d1', auth, adminAuth, async (c) => {
  ...
  try {
    ...
    limit = parseInt(body.limit || '100', 10);
    ...
    for (const object of list.objects) {
      ...
      try {
        // 检查数据库中是否已存在
        const existing = await c.env.DB.prepare('SELECT 1 FROM images WHERE key = ?')
          .bind(key)
          .first();
        ...
        if (meta['expires']) {
          try {
            expiresAt = new Date(parseInt(meta['expires'], 10)).toISOString();
          } catch {
