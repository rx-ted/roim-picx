  ...
  try {
    ...
    if (existing) {
      ...
      await db
        ...
        .bind(existing.id, userData.login, JSON.stringify({ type: 'github_oauth' }))
        .run();
      ...
      await db
        ...
        .bind(userData.login, JSON.stringify({ type: 'github_oauth', first_login: true }))
        .run();
  ...
  try {
    ...
    if (!userEmail) {
      try {
        ...
        const emails =
          await emailRes.json<Array<{ email: string; primary: boolean; verified: boolean }>>();
        // 优先使用已验证的主邮箱
...
authRoutes.post('/checkToken', async (c) => {
    ...
    try {
      ...
      try {
        await c.env.DB.prepare(
          ...
          .bind(systemUser.id, systemUser.login, JSON.stringify({ type: 'token_login' }))
          .run();
      } catch (e) {
  ...
  try {
    ...
    console.log(
      `Check Token User: ${payload.login}, Exp: ${exp}, Date: ${new Date(exp * 1000).toLocaleString()}`,
    );
...
authRoutes.get('/steam/callback', async (c) => {
  ...
  try {
    ...
    // 重定向到前端，带上 token 参数
    return c.redirect(
      `/auth?steam_token=${encodeURIComponent(token)}&steam_user=${encodeURIComponent(JSON.stringify(userPayload))}`,
    );
  } catch (e: any) {
 ...
 * 同步 Steam 用户到 D1 数据库
 */
async function syncSteamUserToDb(
  db: D1Database,
  player: any,
  adminUsers?: string,
): Promise<Partial<User>> {
  ...
  try {
    ...
    if (existing) {
      ...
      await db
        ...
        .bind(player.personaname, player.avatarfull || player.avatar, steamId)
        .run();
      ...
      await db
        ...
        .bind(existing.id, login, JSON.stringify({ type: 'steam_openid' }))
        .run();
      ...
      await db
        ...
        .bind(login, JSON.stringify({ type: 'steam_openid', first_login: true }))
        .run();
  ...
  try {
    ...
    if (existing) {
      ...
      await db
        ...
        .bind(googleUser.name, googleUser.picture, googleUser.email, googleUser.id)
        .run();
      ...
      await db
        ...
        .bind(existing.id, login, JSON.stringify({ type: 'google_oauth' }))
        .run();
      ...
      await db
        ...
        .bind(login, JSON.stringify({ type: 'google_oauth', first_login: true }))
        .run();
...
authRoutes.get('/google/callback', async (c) => {
  ...
  try {
    ...
    const user: User = {
      id: parseInt(googleUser.id.substring(0, 10), 10) || Date.now(),
      login: login,
