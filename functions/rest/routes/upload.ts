...
import { Fail, Build, type ImgItem } from '../type';
import type { User } from '../type';
import { checkFileType, getFileName, hashPassword } from '../utils';
...
uploadRoutes.post('/upload', uploadRateLimit, auth, async (c) => {
  ...
  const isPrivate = files.get('isPrivate') === '1';
  const privatePasswordRaw = files.get('privatePassword')?.toString();

  if (isPrivate && !privatePasswordRaw) {
    return c.json(Fail('Password is required for private images'), 400);
  }

  // Hash password if provided
  let privatePasswordHash: string | null = null;
  if (isPrivate && privatePasswordRaw) {
    privatePasswordHash = await hashPassword(privatePasswordRaw);
  }

  const requestedStorageType = files.get('storageType')?.toString() as 'R2' | 'HF' | undefined;
  const storageType: 'R2' | 'HF' =
    requestedStorageType === 'R2' || requestedStorageType === 'HF'
    ...
    if (object?.key) {
      ...
      if (user) {
        ...
        c.executionCtx.waitUntil(
          c.env.DB.prepare(
            `INSERT INTO images (key, user_id, user_login, original_name, size, mime_type, folder, expires_at, storage_type, tags, nsfw, nsfw_score, is_private, private_password_hash) 
                         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          )
            .bind(
              object.key,
              ...
              fileType,
              customPath || '',
              expireAt ? new Date(parseInt(expireAt.toString(), 10)).toISOString() : null,
              storageType,
              tagsJson,
              nsfw,
              nsfwScore,
            ...
            .then((result) => {
              console.log(
                `[Upload] Image inserted to DB successfully - key: ${object.key}, meta: ${JSON.stringify(result.meta)}`,
              );
              // 更新用户统计
              return c.env.DB.prepare(
                ...
                .bind(finalSize, user.login)
                .run();
            })
            .then((result) => {
              console.log(
                `[Upload] User stats updated - login: ${user.login}, meta: ${JSON.stringify(result.meta)}`,
              );
              // 记录上传审计日志
              return c.env.DB.prepare(
                ...
                .bind(
                  ...
                  user.login,
                  object.key,
                  JSON.stringify({ size: finalSize, originalName: originalName, storageType }),
                )
                .run();
            })
            .catch((e) => {
            ...
            (async () => {
              ...
              if (album) {
                await c.env.DB.prepare(
                  ...
                  .bind(album.id, object.key, storage.getPublicUrl(object.key), time)
                  .run();
