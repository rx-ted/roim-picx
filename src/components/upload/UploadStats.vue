        ...
        <div v-if="convertedImages.length > 0"
            ...
            <div class="text-xs text-gray-500 dark:text-gray-400 space-y-2 mb-6">
                ...
                <div v-if="compressionLevel !== 'none' && compressionRatio < 1" class="flex justify-between">
                    ...
                    <span class="font-bold text-indigo-600 dark:text-indigo-400">{{ Math.round((1 - compressionRatio) *
                        100) }}%</span>
                </div>
            </div>

            <div class="flex flex-col gap-3">
                <BaseButton type="indigo" block @click="$emit('upload')" :loading="loading"
                    :disabled="loading || !isLoggedIn" :tip="isLoggedIn ? undefined : t('common.notLoggedIn')">
