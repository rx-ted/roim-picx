  ...
  <div class="mx-auto max-w-7xl my-8 px-4 sm:px-6 relative min-h-[60vh]">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
      <div>
        ...
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
          {{
            $t("manage.uploadedCount", {
              count: uploadedImages.length,
              size: formatBytes(imagesTotalSize),
            })
          }}
        </p>
      </div>
      <div class="flex flex-wrap items-center gap-3 w-full sm:w-auto">
        ...
        <div class="hidden sm:flex bg-gray-100 dark:bg-gray-800 p-1 rounded-lg mr-2">
          <button class="p-1.5 rounded-md transition-all duration-200 flex items-center justify-center w-8 h-8" :class="viewMode === 'grid'
            ? 'bg-white dark:bg-gray-700 text-indigo-600 dark:text-indigo-400 shadow-sm'
            : 'text-gray-400 hover:text-gray-600 dark:hover:text-gray-200'
            " @click="viewMode = 'grid'" :title="$t('manage.gridView')">
            <font-awesome-icon :icon="faThLarge" />
          </button>
          <button class="p-1.5 rounded-md transition-all duration-200 flex items-center justify-center w-8 h-8" :class="viewMode === 'list'
            ? 'bg-white dark:bg-gray-700 text-indigo-600 dark:text-indigo-400 shadow-sm'
            : 'text-gray-400 hover:text-gray-600 dark:hover:text-gray-200'
        ...
        <BaseButton @click="addFolder" :disabled="!isLoggedIn" :tip="isLoggedIn ? undefined : t('common.notLoggedIn')">
        ...
        <div class="flex items-center gap-2 ml-2 pl-2 border-l border-gray-200 dark:border-gray-700">
          <BaseButton :type="isSelectMode ? 'white' : 'indigo'" @click="toggleSelectMode" :disabled="!isLoggedIn"
            :tip="isLoggedIn ? undefined : t('common.notLoggedIn')">
            <div class="flex items-center gap-2">
              <font-awesome-icon :icon="isSelectMode ? faTimes : faCheckSquare" />
              <span class="hidden sm:inline">{{
                isSelectMode ? $t("common.exit") : $t("manage.batchDelete")
              }}</span>
            </div>
          </BaseButton>
          ...
          <transition name="el-fade-in-linear">
            <div v-if="isSelectMode" class="flex items-center gap-2">
              <button @click="toggleSelectAll"
                class="flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm transition-all duration-200" :class="isAllSelected
              ...
              <BaseButton v-if="hasSelection" type="danger" @click="confirmBatchDelete">
                <div class="flex items-center gap-2">
                  ...
                  <span>{{
                    $t("manage.deleteSelected", { count: selectedKeys.size })
                  }}</span>
                </div>
              </BaseButton>
            </div>
    ...
    <div class="mb-8" v-if="prefixes.length > 0">
      ...
      <div class="flex items-center gap-3 flex-wrap">
        <div v-for="it in prefixes" :key="String(it)"
          ...
          :class="{
            'ring-2 ring-indigo-500 ring-offset-2 dark:ring-offset-gray-900 border-transparent':
              delimiter === it,
      ...
      <div v-if="viewMode === 'grid'"
        ...
        <transition-group name="el-fade-in-linear">
          <div class="relative" v-for="item in uploadedImages" :key="item.url">
            <ManageImageCard :item="item" :selected="selectedKeys.has(item.key)" :is-select-mode="isSelectMode"
              @toggleSelect="toggleSelect(item.key)" @delete="deleteImage(item.key)" @rename="renameImage(item)"
              @detail="showDetailsDialog(item)" @preview="showPreview(item.url)" @share="showShareDialog(item)"
              @addToAlbum="showAddToAlbumDialog(item)" @editTags="showEditTagsDialog(item)"
              @set-private="handleSetPrivate(item.key)" @remove-private="handleRemovePrivate(item.key)"
              class="w-full h-full" />
          </div>
        </transition-group>
      </div>
      ...
      <div v-else class="flex flex-col gap-2">
        <transition-group name="el-fade-in-linear">
          <image-list-row v-for="item in uploadedImages" :key="item.url" :src="item.url" :name="item.key"
            ...
            :uploader-name="item.uploaderName" :tags="item.tags" :nsfw="item.nsfw" :is-private="item.isPrivate"
            :selected="selectedKeys.has(item.key)" :is-select-mode="isSelectMode" @toggleSelect="toggleSelect(item.key)"
            @delete="deleteImage(item.key)" @rename="renameImage(item)" @detail="showDetailsDialog(item)"
            @preview="showPreview(item.url)" @share="showShareDialog(item)" @addToAlbum="showAddToAlbumDialog(item)"
            @editTags="showEditTagsDialog(item)" />
        </transition-group>
      </div>
    ...
    <div v-else-if="!loading" class="py-20 text-center">
      ...
      <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100">
        {{
          searchKeyword ? $t("manage.noSearchResult") : $t("manage.noImages")
        }}
      </h3>
      <p class="mt-1 text-gray-500 dark:text-gray-400">
        {{
          searchKeyword
            ? $t("manage.tryOtherKeyword")
            : $t("manage.emptyFolder")
        }}
      </p>
    ...
    <share-dialog v-model="shareDialogVisible" type="image" :image-key="currentShareImage.key"
      :image-url="currentShareImage.url" :image-name="currentShareImage.name" :image-size="currentShareImage.size" />
    ...
    <BaseDialog v-model="renameDialogVisible" :title="$t('manage.renameImage')" width="400px"
      ...
      <div class="py-2">
        <BaseInput v-model="renameValue" :label="$t('manage.renamePrompt')" :placeholder="$t('manage.renamePrompt')"
          @keyup.enter="handleRenameConfirm" autofocus />
      </div>
    </BaseDialog>
    ...
    <BaseDialog v-model="batchDeleteVisible" :title="$t('manage.confirmDeleteTitle')" width="400px"
      ...
      <div class="py-4 text-center">
        <div
          class="w-16 h-16 bg-red-50 dark:bg-red-900/20 rounded-full flex items-center justify-center mx-auto mb-4 text-red-500">
...
const currentAddToAlbumImages = ref<{ keys: string[]; urls: string[] }>({
  keys: [],
  urls: [],
});
...
const currentEditTagsImage = ref<{ key: string; url: string; tags: string[] }>({
  key: '',
  url: '',
  tags: [],
});
...
const handleSetPrivate = async (key: string) => {
  try {
    const { value: password } = await ElMessageBox.prompt(
      t('image.setPrivate'),
      t('image.private'),
      { inputType: 'password', confirmButtonText: t('admin.loginConfirm') },
    );
    if (password) {
      await setImagePrivate(key, password);
      ElMessage.success(t('image.private'));
      await listImages();
    }
  } catch {
    /* cancelled */
...
const handleRemovePrivate = async (key: string) => {
  try {
    await removeImagePrivate(key);
    ElMessage.success(t('image.removePrivate'));
    await listImages();
  } catch {
    /* failed */
  }
  ...
  try {
    ...
    if (res.code === 200 || res.newKey) {
      ...
      if (index !== -1) {
        ...
        updatedItem.url = updatedItem.url
          .replace(encodeURIComponent(oldKey), encodeURIComponent(updatedItem.key))
