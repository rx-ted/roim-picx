        ...
        <div class="config-collapse-wrapper" :class="{ 'is-collapsed': isConfigCollapsed }">
            <div class="config-collapse-inner">
                <UploadConfig v-model:customPath="customPath" v-model:keepName="keepName"
                    ...
                    v-model:privatePassword="privatePassword"
...
import { ElNotification as elNotify, ElDialog } from 'element-plus';
...
const uploadImages = async () => {
  ...
  if (privatePassword.value) {
    ...
    formData.append('privatePassword', privatePassword.value);
  }
  ...
  requestUploadImages(formData)
    .then((res) => {
      elNotify({
        ...
        message: t('upload.uploadCompleteMsg', {
          count: convertedImages.value.length,
          size: formatBytes(imagesTotalSize.value),
        }),
        type: 'success',
      });
      convertedImages.value = [];
